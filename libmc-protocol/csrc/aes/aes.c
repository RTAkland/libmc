...
#if (defined(CBC) && (CBC == 1)) || (defined(CTR) && (CTR == 1)) || (defined(CFB8) && (CFB8 == 1))
...
void AES_CFB8_encrypt_buffer(struct AES_ctx* ctx, uint8_t* buf, size_t length) {
    ...
    for (size_t i = 0; i < length; ++i) {
        ...
        memmove(ctx->Iv, ctx->Iv + 1, AES_BLOCKLEN - 1);
        ctx->Iv[AES_BLOCKLEN - 1] = ciphertext;
        buf[i] = ciphertext;
    }
}

void AES_CFB8_decrypt_buffer(struct AES_ctx* ctx, uint8_t* buf, size_t length) {
    uint8_t stream[AES_BLOCKLEN];
